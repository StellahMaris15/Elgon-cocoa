...
const Contact = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setErrors({});
    setSubmitting(true);
    // TODO: wire to Cloud edge function in M5
    setTimeout(() => {
      setSubmitting(false);
  ...
  return (
    <Layout>
      ...
      <section className="container-full py-20 grid gap-12 lg:grid-cols-5">
        ...
        <form onSubmit={onSubmit} className="lg:col-span-3 p-8 md:p-10 bg-card border border-border rounded-sm space-y-5" noValidate>
          <div>
            <label className="btn-label text-xs text-primary block mb-2">Name</label>
            <input
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="w-full h-12 px-4 bg-background border border-border rounded-sm focus:border-primary focus:outline-none transition-colors"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 bg-background border border-border rounded-sm focus:border-primary focus:outline-none transition-colors resize-none"
